...
The \pkg{plyr} package makes this straightforward in \proglang{R}. The following code generates a vector of file names in a directory (\code{data/}), matching a regular expression (ends in \code{.csv}). Next we name each element of the vector with the name of the file. We do this because \pkg{plyr} will preserve the names in the next step, ensuring that each row in the final data frame is labelled with its source. Finally, \code{ldply()} loops over each path, reading in the csv file and combining the results into a single data frame.
...
Next to provide informative labels for disease, we join the dataset to the \code{codes} dataset . These two datasets are connected by the \code{cod} variable. This adds a new variable, \code{disease}, shown in Table~\subref*{tbl:counts:2}).
...
We are interested in points that are particularly higher than other points in the graph. Controlling for the number of deaths, these points represent the diseases which depart the most from the overall pattern.
...
\begin{figure}[htbp]
  ...
  \caption{Causes of death with unusual temporal courses. Overall hourly death rate shown in grey. (Top) Causes of death with more than 350 deaths over a year. (Bottom) Causes of death with less than 350 deaths. Note that the y-axes are on different scales.}
  \label{fig:disease}
\end{figure}
