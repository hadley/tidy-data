...
Statistical datasets usually come in the form of a rectangular table, made up of rows and columns. Tables are usually labelled with column names, and sometimes with row names. The table is a collection of data values, typically either numeric (if quantitative) or character (if qualitative). Values are grouped into variables, measurements of a single quantifiable property. Multiple measurements made on the same observational unit form an observation.
